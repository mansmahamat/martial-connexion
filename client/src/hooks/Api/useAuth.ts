...
const postLogin = async (email : string, password: string): Promise<LoginTypes> => {
  const { data } = await axios.post(`${process.env.REACT_APP_SERVER}/user/login`, {
    email,
    password
  });
...
const postRegister = async (email : string, password: string): Promise<RegisterTypes> => {
  const { data } = await axios.post(`${process.env.REACT_APP_SERVER}/api/user/register`, {
    email,
    password
  });
...
const forgotPassword = async (email : string): Promise<ForgotPassword> => {
  const { data } = await axios.post(`${process.env.REACT_APP_SERVER}/user/forgotpassword`, {
    email,
  });
...
const resetPassword = async (password : string, resetToken: string): Promise<ResetPassword> => {
  const { data } = await axios.put(`${process.env.REACT_APP_SERVER}/user/resetpassword/${resetToken}`, {
    password,
  });
...
const getUser = async (id: string) => {
  const { data } = await axios.get(`${process.env.REACT_APP_SERVER}/user/user/${id}`)
